...
public class Benchmark {
  ...
  public static double Mark5() {
    ...
    do {
      ...
      for (int j=0; j<n; j++) {
        Timer t = new Timer();
        for (int i=0; i<count; i++) 
          dummy += multiply(i); //要测试的方法
        runningTime = t.check();
        double time = runningTime * 1e9 / count; //也是平均数
        st += time; 
        sst += time * time;
        totalCount += count;
      }
      double mean = st/n, sdev = Math.sqrt((sst - mean*mean*n)/(n-1)); // mean是所有平均数的总和除以次数
  ...
  public static double Mark7(String msg, IntToDoubleFunction f) {
    ...
    double mean = st/n, sdev = Math.sqrt((sst - mean*mean*n)/(n-1));
    System.out.printf("%-25s %15.1f ns %10.2f %10d%n", msg, mean, sdev, count); //只打印最终结果，如果iterate 8388608次，就只打印那次的。
    return dummy / totalCount;
  }
