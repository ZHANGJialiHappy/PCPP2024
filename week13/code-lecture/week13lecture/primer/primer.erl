...
handle_check_prime(Number, State = #primer_state{workers = Workers}) ->
    %%比如8余3=2，就从Workers中返回编号2的worker。（8是要检查的数字，3是worker的数量）
...
check_primes(N, NumWorkers, MaxNumber) ->
    ...
    lists:map(fun (X) -> 
                      ...
                      primer:check_prime(PID, RandomNumber), %primer:check_prime(PID, X, RandomNumber),
                      {X, RandomNumber} %如果想要按照顺序打印，可以把X也发送到worker,然后X和检查结果发送回来PrimeServer，最后按照X大小打印结果。
                  end,
                  lists:seq(1,N)).
