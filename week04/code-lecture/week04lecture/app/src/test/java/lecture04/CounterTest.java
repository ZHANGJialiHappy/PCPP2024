...
// 1. 明确定义要测试的属性 (Precisely define the property you want to test)，这里是测试safety property
    ...
    public void initialize() {
        //2. 定义接口，便于测试多个实现 (Define an interface for multiple implementations)
        count = new CounterDR();
    ...
    @MethodSource("argsGeneration") // 4. Run the tests multiple times
    // 3. 设置多线程环境 (Setup for multiple threads) 
    ...
    @RepeatedTest(1000) // 4. Run the tests multiple times
    // @Test
    // @Disabled
        ...
        for (int j = 0; j < nrThreads; j++) {
            ...
            new Thread(() -> {
                    try {
                        ...
                        //某个线程结束，主线程中继续执行，提前assert，会导致assertTrue结果错误
                    } catch (InterruptedException | BrokenBarrierException e) {
                        e.printStackTrace();
                    }
